...
public class DynamicBone : MonoBehaviour
{
    ...
    // Runtime-only force supplied by components such as DynamicBoneWind.
    // Kept separate from m_Force so external effects never accumulate into
    // the force configured on this Dynamic Bone component.
    [HideInInspector]
    ...
    void UpdateParticles1(ParticleTree pt, float timeVar, int loopIndex)
    {
        ...
        force = (force + m_Force + m_ExternalForce) * (m_ObjectScale * timeVar);
